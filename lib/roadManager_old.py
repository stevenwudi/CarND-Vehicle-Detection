...
class RoadManager:
    ...
    def __init__(self, camCal, keepN=10, debug=False, scrType=0):
        ...
        ################  vehicles ##################
        #############################################
        self.vehicleDetection = VehicleDetection(self.projMgr.projectedX, self.projMgr.projectedY)

        self.vehicleTracking = VehicleTracking(self.x, self.y,
                                               self.projMgr.projectedX, self.projMgr.projectedY, self.lanes)
    ...
    def addLaneLeft(self, curLane):
        ...
        if self.resized:
            faint *= 0.75
        if ((np.absolute(faint) > 0.5 or (self.resized and np.absolute(faint) > 0.4)) and
                curLane.adjacentLeft and curLane.adjacentLLane is None):
            # print("threshold:", faint)
            newLeftLine = Line(self.left, self.x, self.y,
                               self.projMgr.projectedX,
                               self.projMgr.projectedY,
                               self.maskDelta)
            newLeftLine.createPolyFitLeft(self.curImgFtr, curLane, faint=faint, resized=self.resized)
    ...
    def findLanes(self, img, resized=False):
        ...
        # low confidence?
        if mainLane.confidence() < 0.5 or self.curFrame < 1:
            self.restartCount += 1
        ...
        self.specialProjectedEffects = self.curImgFtr.miximg(self.roadsurface * 0,
                                                             self.specialProjectedEffects, 1.0, 0.9)
    ...
    def drawLaneStats(self, color=(224, 192, 0)):
        ...
        if self.lineBasePos < 0.0:
            ...
            cv2.putText(self.final, text %
                        (-self.lineBasePos * 100), (30, 90), self.font, 1, color, 2)
            ...
            cv2.putText(self.final, text, (30, 90), self.font, 1, color, 2)
    ...
    def findVehicles(self):
        ...
        for vehIdx in range(len(self.vehicles)):
            ...
            y1 = 150 + 120 * vehIdx
            y2 = y1 + self.selfie_size[1]
            # print("y1, y2: ", y1, y2)
            if y2 < 720:
                if len(self.vehicles[vehIdx].selfie.shape) > 2:
                    car = cv2.resize(self.vehicles[vehIdx].selfie.astype(np.uint8), self.selfie_size, interpolation=cv2.INTER_AREA)
                    car = cv2.cvtColor(car, cv2.cv2.COLOR_BGR2RGB)
                    self.final[y1:y2, startx:startx + self.selfie_size[0]] = car
        ...
        if self.debug:
            ...
            cv2.putText(self.projMgr.diag4, 'Frame: %d' % self.curFrame, (30, 30), self.font, 1, (255, 255, 0), 2)
            cv2.putText(self.diag1, 'Frame: %d' % self.curFrame, (30, 30), self.font, 1, (255, 0, 0), 2)
            for i in range(len(self.lines)):
                ...
                cv2.putText(self.projMgr.diag4, text %
                            ...
                            (30, 60 + 90 * i), self.font, 1, (255, 255, 0), 2)
                if (self.lines[i].radiusOfCurvature is not None and self.lines[i].lineBasePos is not None):
                    ...
                    cv2.putText(self.projMgr.diag4, text %
                                ...
                                (30, 90 + 90 * i), self.font, 1, (255, 255, 0), 2)
                ...
                cv2.putText(self.projMgr.diag4, linetext,
                            (30, 120 + 90 * i), self.font, 1, (255, 255, 0), 2)

            if self.boosting > 0.0:
                y = 90 * len(self.lines)
                cv2.putText(self.projMgr.diag4, 'Boosting @ %f%%' % self.boosting, (30, 60 + y), self.font, 1,
                            (128, 128, 192), 2)
                    ...
                    for text in Text.split('\n'):
                        if i == 0:
                            cv2.putText(
                                ...
                                (startx, 265 + 120 * vehIdx), self.font, 0.60,
                                self.vehicles[vehIdx].statusColor, 2)
                        else:
                            cv2.putText(
                                self.projMgr.diag2, text,
