#!/usr/bin/python
"""
Python class that handles image analysis and filtering operations
"""
...
class ImageFilters:
    ...
    def __init__(self, camCal, projectedX, projectedY,
        ...
        self.curRoadEdgeProjected = np.zeros((self.projectedY, self.projectedX, 3), dtype=np.uint8)

        # current Projected image
        self.curRoadProjected = np.zeros((self.projectedY, self.projectedX, 3), dtype=np.uint8)
    ...
    def hls_h(self, hls, thresh=(0, 255)):
        # 1) Convert to HLS color space
        # 2) Apply a threshold to the S channel
        # 3) Return a binary image of threshold result
        h = hls[:, :, 0]
    ...
    def edges(self):
        """
         retrieve edges detected by the filter combinations (used by other modules to locate current binary image.)
        :return:
        """
    ...
    def imageQ(self, image):
        self.curImage = cv2.undistort(image, self.mtx, self.dist, None, self.mtx).astype(np.float32)
        self.yuv = cv2.cvtColor(self.curImage, cv2.COLOR_RGB2YUV).astype(np.float32)
    ...
    def horizonDetect(self, debug=False, thresh=50):
        ...
        if not self.horizonFound:
            ...
            while not self.horizonFound and horizonLine < int(self.y / 2):
                magchlinesum = np.sum(self.magch[horizonLine:(horizonLine + 1), :]).astype(np.float32)
    ...
    def applyFilter2(self):
        # Run the functions
        img = np.copy(self.curRoadRGB).astype(np.uint8)
        self.curRoadGray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
        self.curRoadHLS = cv2.cvtColor(img, cv2.COLOR_RGB2HLS)
        ...
        # build the combination
        combined = np.zeros_like(dirch).astype(np.uint8)
        combined[((gradx > 0) | (grady > 0) | ((self.magch > 0) & (dirch > 0)) | (
            sch > 0)) & (shadow == 0) & (rEdgeDetect > 0)] = 35
        combined[(grady > 0) & (dirch > 0) & (self.magch > 0)] = 35
        self.curRoadEdge = combined
        ...
        if self.debug:
            ...
            ignore_color = np.copy(gradx) * 0
            self.diag1 = np.dstack((rEdgeDetect, gradx, grady))
            self.diag2 = np.dstack((ignore_color, self.magch, dirch))
            self.diag3 = np.dstack((sch, shadow, hch))
            self.diag4 = np.dstack((combined, combined, combined)) * 4
