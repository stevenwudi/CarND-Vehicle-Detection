...
class ProjectionManager:
    ...
    def hough_lines(self, img, rho, theta, threshold, min_line_len, max_line_gap, backoff=30, debug=False):
        """
    ...
    def unwarp_lane(self, img, src, dst):
        """
        ...
        :param img:
        :param src: np.float32([[,],[,],[,],[,]])
        :param dst: np.float32([[,],[,],[,],[,]])
        :param mtx:
        :return:
        """
    ...
    def unwarp_lane_back(self, img):
        """
        unction to project the undistorted camera image to a plane looking down.
        :param img:
        :return:
        """
        img_size = (self.x, self.y)
        warped = cv2.warpPerspective(img, self.dst2srcM, img_size, flags=cv2.INTER_LINEAR)
        return warped, self.dst2srcM

    def find_lane_locations(self, masked_lines):
        """
        function to find starting lane line positions return left and right column positions
        :param masked_lines:
        :return:
        """
    ...
    def findInitialRoadCorners(self, imgftr):
        ...
        threshold = 40
        # minimum number of pixels making up a line [125, 100, 75, 50, 25]
        min_line_length = 125
        # 40 50 20 maximum gap in pixels between connectable line segments [45, 40, 35, 30, 20]
        max_line_gap = 45
        lane_info = [-1000]

        while lane_info[0] == -1000 and self.hough <= 5:
            self.hough += 1
            min_line_length -= 25
            max_line_gap -= 5
            line_image, lane_info = self.hough_lines(masked_edge, rho, theta, threshold, min_line_length, max_line_gap)
        ...
        if lane_info[0] > -1000:
            ...
            left = -(self.lane_width / 2) * self.scale_factor
            right = (self.lane_width / 2) * self.scale_factor
    ...
    def resetDestTop(self, projectionTopPixel):
        ...
        approx_dest = 42.0 * (1.0 - ((projectionTopPixel / self.projectedY) * 0.50))
