#!/usr/bin/python
"""
Python class that handles line detection, measurements and confidence calculations and operations.
There is now a shared array of instances of this class: lines.
"""
    ...
    def getLineStats(self, img, faint=1.0, resized=False):
        ...
        if (red > np.absolute(200 * faint) and green < np.absolute(200 * faint) and blue < np.absolute(200 * faint)) \
            ...
            if faint == 1.0:
                ...
                self.line_color = "white"
        elif red > np.absolute(200 * faint) and green > np.absolute(200 * faint) and blue > np.absolute(200 * faint):
        ...
        # left solid yellow
        if self.lineType == "solid" and self.line_color == "yellow" and self.side == 1:
            self.adjacentLeft = False
            self.adjacentLLine = None
            self.adjacentRight = True
            self.adjacentRLine = None
        elif self.lineType == "solid" and self.line_color == "white" and self.side == 2:
